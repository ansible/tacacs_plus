...
class TACACSClient(object):
        ...
        if authen_type in (
            ...
            if authen_type == TAC_PLUS_AUTHEN_TYPE_CHAP:
                ...
                if len(chap_challenge) > 255:
                    raise ValueError(
                        'chap_challenge may not be more 255 bytes'
                    )
                start_data = (
                    six.b(chap_ppp_id)
                    + six.b(chap_challenge)
                    + md5(
                        six.b(chap_ppp_id + password + chap_challenge)
                    ).digest()
                )
        with self.closing():
            packet = self.send(
    ...
    def authorize(
        ...
        with self.closing():
            ...
            user_priv_lvl = int(
                reply_arguments.get(six.b('priv-lvl'), TAC_PLUS_PRIV_LVL_MAX)
            )
...
class AsyncTACACSClient(object):
    """
    A TACACS+ authentication client.
    https://datatracker.ietf.org/doc/draft-ietf-opsawg-tacacs
    ...
    async def make_reader_writer(self):
        conn = (self.host, self.port)
        if self.family == socket.AF_INET6:
            # For AF_INET6 address family, a four-tuple (host, port,
            # flowinfo, scopeid) is used
    ...
    async def authenticate(
        ...
        if authen_type in (
            ...
            if authen_type == TAC_PLUS_AUTHEN_TYPE_CHAP:
                ...
                if len(chap_ppp_id) != 1:
                    raise ValueError(
                        'chap_ppp_id must be a 1-byte string'
                    )
                if not isinstance(chap_challenge, str):
                    raise ValueError(
                        'chap_challenge must be a string'
                    )
                if len(chap_challenge) > 255:
                    raise ValueError(
                        'chap_challenge may not be more 255 bytes'
                ...
                start_data = (
                    bytes(chap_ppp_id)
                    + bytes(chap_challenge)
                    + md5(
                        bytes(chap_ppp_id + password + chap_challenge)
                    ).digest()
                )
                print(type(start_data))
        async with self.make_reader_writer() as (reader, writer):
    ...
    async def authorize(
        ...
        async with self.make_reader_writer() as (reader, writer):
            ...
            user_priv_lvl = int(
                reply_arguments.get(b'priv-lvl', TAC_PLUS_PRIV_LVL_MAX)
            )
            if user_priv_lvl < priv_lvl:
                reply.status = TAC_PLUS_AUTHOR_STATUS_FAIL
